...
```
module plc.lambda.BigStep where
```
...
open import plc.lambda.Untyped
  ...
  —↠-trans; appL-cong)
open import plc.lambda.Substitution using (Subst; ids)
```
...
sub-id = plc.lambda.Substitution.sub-id
```
...
subst-zero-exts {Γ}{Δ}{σ}{B}{M}{x} =
   cong-app (plc.lambda.Substitution.subst-zero-exts-cons{σ = σ}) (S x)
```
...
sub-sub {M = M} = plc.lambda.Substitution.sub-sub {M = M}
```
