...
```
module plfa.lambda.BigStep where
```
...
open import plfa.lambda.Untyped
  ...
  —↠-trans; appL-cong)
open import plfa.lambda.Substitution using (Subst; ids)
```
...
sub-id = plfa.lambda.Substitution.sub-id
```
...
subst-zero-exts {Γ}{Δ}{σ}{B}{M}{x} =
   cong-app (plfa.lambda.Substitution.subst-zero-exts-cons{σ = σ}) (S x)
```
...
sub-sub {M = M} = plfa.lambda.Substitution.sub-sub {M = M}
```
